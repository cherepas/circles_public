...
parser.add_argument('-transappendix', type=str, default="_img_path")
parser.add_argument('-minmax_fn', type=str, default='mean,std')
...
def train_model(model, dataloaders, samplers, criterion, optimizer,
    ...
    if rank == 0:
        ...
        #      int(len(dataloaders[phase])%opt.bs
        #      ))
        #     print('%s consists of %d batches with size %d, ' +\
        #     'the last batch has size of %d'\
        #      %(phase,
        #      int(len(dataloaders[phase])/opt.bs),
        #      opt.bs,
        #       int(len(dataloaders[phase])%opt.bs)
    ...
    F_Nw = np.array(h5py.File(os.path.join(
        mainpath, 'F_N_' +
        opt.csvname+'.h5').replace('\\', '/'), 'r').get('dataset'))
    F_Nw = F_Nw[:,:opt.ampl]
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i_batch, sample_batched in enumerate(dataloaders[phase]):
                ...
                # print(pathes)
                sz = sample_batched['image'].shape
                img = torch.reshape(sample_batched['image'],
                    (sz[0]*sz[1], sz[2], sz[3]))
...
#                     _, preds = torch.max(outputs, 1)
            ...
            if opt.wandb:
                wandb.log({phase+' loss': lossar[phasecnt][epoch]})
#                 if lb == 'pc':
#                 lossar[:][epoch] *= vox2mm
            if mt:
...
#                 gt0 = gt.reshape((3, nsp))
                ...
                plt.savefig(dirname + 'pc_'+phase+'_' +\
                            str(cnt).zfill(3) + '.png')
                plt.close(fig)
                curloss[phasecnt,epoch] = np.mean(np.abs(oo-gt0))
            if rank == 0 and lb == 'pc' and\
...
#                 print(lossars)
            ...
            if rank == 0:
                print('{} Loss: {:.6f}'.format(phase, lossar[phasecnt][epoch]))
                print()
        cnt += 1
        fte = time.time()
        if rank == 0:
...
es0 = "transforms.Compose([" +\
     minmax3dimage + normalize + center +\
     cmscrop + downsample  + rescale +\
     "ToTensor(iscuda)])"
es = ("data_transforms = {\'train\': "+es0+",'val': "+es0+"}")
...
#         bsa[phasecnt, i_batch] = img.shape[0]
# print(bsa)
if mt:
