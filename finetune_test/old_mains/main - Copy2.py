...
#                         print('model time = ',tf-ts)
                    ...
                    #                 (int(angle/10) +
                    #                  1)*nsp, :]
                    if (lb == 'f' or lb == 'pc' or lb == 'pc+f') and\
                        enim == 1 and\
                    ...
                    if lb == 'pc' or lb == 'f':
                        ...
                        far = torch.zeros(inputs.shape[0],nsp)
...
#                     _, preds = torch.max(outputs, 1)
                ...
                bft = time.time()
                if rank == 0:
                    print('batch %d was done in %f seconds' %(i_batch, bft-bst))
            ...
            elif phase == 'train' and lb == 'pc+f':
                lossar[0][epoch] = rloss0/loss_cnt
                lossar[2][epoch] = rloss1/loss_cnt
            elif phase == 'val' and lb == 'pc+f':
                lossar[1][epoch] = rloss0/loss_cnt
                lossar[3][epoch] = rloss1/loss_cnt
            elif phase == 'train' and not lb == 'pc':
                lossar[0][epoch] = rloss/loss_cnt
...
#                     np.trim_zeros(lossar[1,:]), ])
                ...
                plt.close(fig)
                np.savetxt(dirname+'lt.npy',lt)
#                 print(lossars)
                lossfig(dirname, lossar, 'Loss', (0,epoch), (0,20), lb)
                logloss = np.ma.log10(lossar)
                lossfig(dirname, logloss.filled(0), 'log10(Loss)', (0,epoch), (0,0), lb)
            if (rank == 0 and
                    epoch == epoch0+1 and
                    opt.save_output and
                    (lb == 'f' or
...
def lossfig(dirname, lossar, ylabel, xlim, ylim, lb):
