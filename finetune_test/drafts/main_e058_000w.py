...
if __name__ == '__main__':
    ...
    parser.add_argument('-steplr',  nargs='+', default=(30,1))
    parser.add_argument('-outputt', type=str, default='points')
    ...
    def trainit(num_epochs, lframe, train_part, bs, dataloaders, mainpath, nsp, classicnorm, csvname, ampl, dir1,
                localexp, save_output, rank, path1, rot_dirs, merging_order, iscuda):
        val_acc_history = []
        ...
        if rank == 0:
            ...
            for pcnt, phase in enumerate(['train', 'val']):
                ...
                ebn.append(len(dataloaders[phase])-1 if lfl[pcnt]%opt.bs else len(dataloaders[phase]))
        ...
        F_Nw = np.array(h5py.File(jn(
            mainpath, csvname+'_F_N.h5').replace('\\', '/'),
                'r').get('dataset'))
        prmatw = np.array(h5py.File(jn(
            mainpath, opt.specie+'prmat.h5').replace('\\', '/'),
                'r').get('dataset'))
        bigm = np.copy(prmatw)
        ...
        if opt.save_output and rank == 0 and not opt.localexp:
            ...
            while dirname is None:
                try:
                    ...
                    time.sleep(1)
                    pass
        elif localexp and save_output and rank==0:
            dirname = jn(dir1,localexp)
        ...
        # print(bX.shape)
        return y_n, bX, F_Nw, tmean, dirname, bX, C, y_n, y_n2, dirs, \
               prmatw, bigm, matw
    def train_model(model, dataloaders, samplers, criterion, optimizer,
        ...
        lossarb = np.zeros([2, num_epochs])
        curloss = np.zeros([2, num_epochs])
        lt = np.zeros([4, len(dataloaders['train'])])
        cnt = 0
        y_n, bX, F_Nw, tmean, dirname, bX, C, y_n, y_n2, dirs, prmatw,\
        bigm, matw =\
            trainit(num_epochs, lframe, train_part, opt.bs, dataloaders,
                    mainpath, nsp, classicnorm, opt.csvname, opt.ampl,
                dir1, opt.localexp, opt.save_output, rank, path1, opt.rot_dirs,
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for i_batch, sample_batched in enumerate(dataloaders[phase]):
                    ...
                    with t.set_grad_enabled(phase == 'train'):
                        ...
                        if (opt.inputt == 'img' or\
                                opt.inputt == 'f') and\
                                not lb == 'pc+f' and opt.outputt != 'f_n':
                    ...
                    if cond and opt.lb == 'f' and opt.outputt == 'f_n':
                        ...
                        showmanypoints(cbs,nim,ob,gtb,pathes,angles_list,phase,
                            i_batch,cnt,jn(dirname,'perbatch_showpoint'),
                ...
                #         tmean[3, :ampl] -
                #         tmean[2, :ampl]) + tmean[2, :ampl]
                if log1 and not opt.minmax and opt.outputt != 'f_n':
                ...
                if all([rank == 0, epoch == epoch0+1, opt.save_output,
                    ...
                    try:
                        ...
                        savelist([str(int(i/10)) for i in\
                            ...
                            jn(dirname,"angles_"+phase+".txt"))
                if all([log1, lb != 'f_n', epoch == epoch0+1]):
                    # try:
                    #     print(gt.shape)
                    # except:
                    #     print(gt[0].shape)
                    np.savetxt(jn(dirname,'gt_'+phase),
                ...
                if rank == 0 and epoch == epoch0+1 and opt.save_output:
                    for n in opt.netname:
                        shutil.copy(jn(homepath, 'circles/finetune_test/experiments',
                                                 opt.expnum, n+'.py'),
                                                    jn(dirname,n+'.py'))
                    shutil.copy(jn(homepath, 'circles/finetune_test/main.py'),
                                jn(dirname, 'main.py'))
                    shutil.copy(jn(path1, "transform"+opt.transappendix+".py"),
                ...
                #     oo = o[0]
                if all([rank == 0, lb != 'f_n', opt.save_output, epoch > epoch0]):
                    ...
                    showmanypoints(cbs,nim,o.reshape((-1,3,nsp)),vox2mm*gt[pcnt],
                        pathes,angles_list,phase,i_batch,cnt,dirname,
                        opt.merging_order, vox2mm)
    ...
    #                 print(lossars)
                if (rank == 0 and
                    ...
                    np.savetxt(jn(dirname,'o_'+phase+'_' +\
                ...
                if all([rank == 0, epoch > epoch0, opt.save_output,
                        opt.outputt != 'f_n']):
                    np.savetxt(jn(dirname,'latent_'+phase+'_' +\
    ...
    exec(open(jn(path1, "transform"+opt.transappendix+".py")).read())
    ...
    excpath = jn(path1,opt.specie+'_exceptions.txt')
    # mtl(mt1)
    if not os.path.isfile(excpath) and not opt.single_folder:
        cip = []
        # i = 0
        for root, directories, filenames in os.walk(jn(mainpath, opt.csvname)):
            for filename in filenames:
    ...
    # print(len(exception_list))
    # sys.exit()
    pts = jn(path1, 'csv', opt.specie + 'frame.csv')
    print('pts',pts)
    ...
    if inn and not opt.rand_angle and enim > 1:
        ...
        alf = [0]
    # print(alf)
    pts = jn(path1, 'csv', opt.specie + '_view_sep_'+str(nim)+'.csv')
    ...
    scheduler = StepLR(optimizer, step_size=float(opt.steplr[0]), gamma=float(opt.steplr[1]))
    if opt.parallel == 'hvd':
