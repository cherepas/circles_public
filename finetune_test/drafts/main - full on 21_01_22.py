...
if __name__ == '__main__':
    ...
    parser.add_argument('-merging_order', type=str,
        choices=['color_channel', 'latent_merge', 'separate'], default='latent_merge')
    ...
    parser.add_argument('-gttype', type=str,
                        choices=['single_f_n', 'single_file'],
                        default='single_file')
    ...
    parser.add_argument('-outputt', type=str, choices=['points','pose6', 'eul'], default='points')
    ...
    if opt.machine == 'jureca':
        ...
        homepath = 'C:/'
    homepath = jn(homepath, 'cherepashkin1')
    # dir1 = jn(homepath, '598test', 'plot_output',opt.expnum)
    dir1 = jn(homepath, 'circles', 'finetune_test', 'plot_output', opt.expnum)
    # print('227')
    ...
    # dir1 = jn(homepath, 'cherepashkin1'
    finePath = jn(homepath, 'circles', 'finetune_test')
    dataPath = jn(homepath, 'phenoseed', opt.specie)
    ...
    def trainit(lframe, train_part, bs, nsp, classicnorm, csvname, ampl,
        ...
        C, y_n, F_Nw, prmatw, bigm, matw, GTw = [t.Tensor(i) for i in\
                                            [C, y_n, F_Nw, prmatw, bigm, matw, GTw]]
        if iscuda:
        ...
        if lb=='eul' and opt.merging_order == 'separate':
        ...
        for epoch in range(opt.epoch):
            ...
            for pcnt, phase in enumerate(['train', 'val']):
                ...
                for i_batch, sample_batched in enumerate(dataloaders[phase]):
                    ...
                    #absolute index
                    index = t2int(sample_batched[2].tolist())
                    ...
                    sz = inputs.shape
                    cnum = sz[1]
                    if all([lb in ('pc', 'f'), opt.gttype in ('single_f_n', 'single_file'),
                        opt.merging_order != 'color_channel']):
                        pathes = [j for j in pathes for i in range(enim)]
                        angles_list = t.flatten(angles_list)
                    if all([lb in ('pc', 'f'), opt.gttype == 'single_f_n']):
                        # print(pathl)
                        ...
                        for i, angle in enumerate(angles_list):
                          dirs[i] = \
                                bX[:, int(angle/10)*2:
                                    (int(angle/10) +
                                     1)*2]
                    if opt.outputt == 'eul' and lb == 'eul':
                        # GT = t.Tensor(lframe.loc[index,
                        #     ['eul' + str(i) for i in range(3)]].values)
                        GT = GTw[index]
                    if lb in ('pc', 'pc+f'):
                        # making pc GT
                ...
                if all([rank == 0, epoch > 0, opt.save_output, lb == 'eul']):
    ...
    image_datasets = {x: Seed3D_Dataset(
        jn(homepath, 'phenoseed'), df_dict[x],
