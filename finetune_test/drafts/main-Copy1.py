                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if opt.loss_between == 'f' and\
                        ...
                        for i, angle in enumerate(angles_list):
                            y_n2[i] = \
                                y_n[int(angle/10)*opt.num_sam_points:
                                    (int(angle/10) +
                                     1)*opt.num_sam_points, :]
                    elif opt.loss_between == 'f' and\
                        opt.num_input_images == 1 and\
                            opt.zero_angle:
                        y_n2 = \
                            torch.unsqueeze(y_n[:opt.num_sam_points,
                                                :], axis=0)
                        y_n2 = \
                            y_n2.repeat(inputs.shape[0], 1, 1)
                    if not opt.num_input_images == 1:
...
#                     _, preds = torch.max(outputs, 1)
            ...
            log1 = (rank == 0 and phase == 'val' and
                    epoch % 10 == 0 and opt.loss_between == 'f' and
                    opt.save_output)
...
#                                                         axis=2)
        ...
        if not opt.noise_output:
            ...
            far = torch.rand(opt.num_sam_points)
