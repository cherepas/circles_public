...
if __name__ == '__main__':
    ...
    parser.add_argument('-merging_order', type=str,
        choices=['color_channel', 'latent_merge', 'batch_channel'], default='batch_channel')
    ...
    def trainit(lframe, train_part, bs, nsp, classicnorm, csvname, ampl,
        ...
        if opt.outputt == 'eul'\
        ...
        elif opt.outputt == 'eul' and opt.merging_order == 'batch_channel':
            ...
            # print(610, GTw.shape)
            #TODO understand, around which axis there is rotation, and
            # add this to the eul angles directly
        ...
        elif opt.outputt == 'orient' and opt.merging_order == 'color_channel':
        ...
        if opt.outputt == 'eul' and\
                opt.merging_order == 'batch_channel' and opt.minmax_fn:
            gstat = np.array([np.min(GTw,axis=(0,1)), np.max(GTw,axis=(0,1)),
                              np.mean(GTw,axis=(0,1)), np.std(GTw,axis=(0,1))])
        if opt.outputt == 'eul' and\
                opt.merging_order == 'color_channel' and opt.minmax_fn:
        ...
        if lb in ('eul', 'orient') and opt.merging_order == 'batch_channel':
    ...
    if opt.num_input_images > 1 and opt.merging_order == 'batch_channel':
        cnum = 1
    else:
