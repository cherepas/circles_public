...
#echo "$EXPNUM"
python -u "${HOMEPATH}/circles/finetune_test/main.py" -epoch 1000 -bn 6 -bs 80 -localexp "" -lr 1e-4 -expnum "$EXPNUM" -hidden_dim "hidden_dim = np.hstack((np.repeat(128, 1),500,1000,1500))" -inputt "img" -num_input_images 3 -lb "f" -zero_angle -gttype "single_f_n" -outputt "pc" -csvname "598csv11" -parallel "torch" -machine "workstation" -wandb "" -merging_order 'color_channel' -updates_per_epoch 1 -steplr 30 0.1 -batch_output 10
